...
CANONICAL_TASKS = {'planner', 'builder', 'checker', 'writer'}
CANONICAL_REPORTS = {'planner_report', 'builder_report', 'checker_report', 'writer_report'}
...
def get_active_cmd_ids():
    """Return command IDs in lead_to_builder that are not done."""
    queue_dir = get_queue_dir()
    shogun_file = queue_dir / 'lead_to_builder.yaml'
...
def slim_lead_to_builder():
    """Archive done/cancelled commands from lead_to_builder.yaml."""
    queue_dir = get_queue_dir()
    archive_dir = queue_dir / 'archive'
    shogun_file = queue_dir / 'lead_to_builder.yaml'
    ...
    archive_timestamp = get_timestamp()
    archive_file = archive_dir / f'lead_to_builder_{archive_timestamp}.yaml'
...
def main():
    ...
    # Process lead_to_builder if this is Karo
    if agent_id == 'lead':
        if not slim_lead_to_builder():
            sys.exit(1)
        migration(dry_run)
