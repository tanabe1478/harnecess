...
# Example: bash scripts/inbox_watcher.sh builder harnecess-agents:agents.1 claude
#
# 設計思想:
...
if [ "${__INBOX_WATCHER_TESTING__:-}" != "1" ]; then
    ...
    if [[ "$CLI_TYPE" == "claude" ]]; then
        touch "${IDLE_FLAG_DIR:-/tmp}/harnecess_idle_${AGENT_ID}"
...
disable_normal_nudge() {
    ...
    if [ -f "${IDLE_FLAG_DIR:-/tmp}/harnecess_idle_${AGENT_ID}" ]; then
...
try:
    ...
    # This prevents restarting a task that Lead intentionally cancelled via clear_command.
...
send_cli_command() {
    ...
    # Safety: never inject CLI commands into the lead pane.
    # Lead is controlled by the User; keystroke injection can clobber human input.
    if [ "$AGENT_ID" = "lead" ]; then
        echo "[$(date)] [SKIP] lead: suppressing CLI command injection ($cmd)" >&2
        return 0
    fi
...
send_context_reset() {
    ...
    # Only builder should receive automatic context resets (clear stale task context).
    # Lead (human-controlled), Lead (coordinator state), Gunshi (strategic state)
    # all maintain complex running context that should not be wiped automatically.
    if [ "$AGENT_ID" = "lead" ] || [ "$AGENT_ID" = "lead" ] || [ "$AGENT_ID" = "planner" ]; then
...
agent_is_busy() {
    ...
    if [[ "$effective_cli" == "claude" ]]; then
        ...
        [ ! -f "${IDLE_FLAG_DIR:-/tmp}/harnecess_idle_${AGENT_ID}" ]
    else
...
#   watching (e.g. single-pane lead session where pane_is_active is always true).
...
send_wakeup() {
    ...
    # Exception: lead — inbox must be delivered immediately regardless of busy state.
    if agent_is_busy && [[ "$AGENT_ID" != "lead" ]]; then
    ...
    # Lead: deliver nudge via send-keys like other agents.
    # inbox messages must reach Claude Code directly.
...
send_wakeup_with_escape() {
    ...
    # Safety: never send Escape escalation to lead. It can wipe the User's input.
    if [ "$AGENT_ID" = "lead" ]; then
        echo "[$(date)] [SKIP] lead: suppressing Escape escalation; sending plain nudge" >&2
...
process_unread() {
    ...
    if no_idle_full_read "$trigger" && [ "$fast_count" -eq 0 ] 2>/dev/null; then
        ...
        touch "${IDLE_FLAG_DIR:-/tmp}/harnecess_idle_${AGENT_ID}" 2>/dev/null || true
        if ! agent_is_busy; then
            # Lead: only clear input when pane is not active (User is away)
            if [ "$AGENT_ID" = "lead" ] && pane_is_active; then
                : # User may be typing — skip C-u
            else
...
" 2>/dev/null)
    ...
    # 案B+待機: Lead がタスク YAML を cancelled に更新するまでの猶予を確保してから
    # status チェックを行い、cancelled/idle の場合はスキップする。
    # clear_sent（実際に送信）のみauto-recoveryを起動。busy時スキップは対象外。
    if [ "$clear_sent" -eq 1 ]; then
        # Wait for Lead to update task YAML status (cancellation race condition mitigation).
    ...
    if [ "$normal_count" -gt 0 ] 2>/dev/null; then
        ...
        # Exception: lead — inbox must be delivered immediately.
        ...
        #          (3) agent is lead (human-controlled).
        ...
        if [ "$age" -lt "$ESCALATE_PHASE1" ]; then
            ...
            if [ "$LAST_CLEAR_TS" -lt "$((now - ESCALATE_COOLDOWN))" ]; then
                ...
                if [[ "$effective_cli" == "codex" ]]; then
                    ...
                    send_wakeup "$normal_count"
                elif [ "$AGENT_ID" = "lead" ] || [ "$AGENT_ID" = "lead" ] || [ "$AGENT_ID" = "planner" ]; then
                    # Command-layer agents (lead/planner): suppress /clear even in Phase 3
