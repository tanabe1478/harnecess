...
#   2. Auto-notifies lead via inbox_write (background, non-blocking)
...
# Lead is the Lord's conversation pane — skip stop hook entirely
...
if [ "$STOP_HOOK_ACTIVE" = "True" ]; then
    ...
    FLAG="${IDLE_FLAG_DIR:-/tmp}/lead_idle_${AGENT_ID}"
    touch "$FLAG"
...
# Lead skips lead notification (lead doesn't report to lead)
...
if [ -n "$LAST_MSG" ]; then
    ...
    # Send notification to lead (background, non-blocking)
    # Lead doesn't report to lead — skip notification
    if [ -n "$NOTIFY_TYPE" ] && [ "$AGENT_ID" != "lead" ]; then
        bash "$SCRIPT_DIR/scripts/inbox_write.sh" lead \
            "$NOTIFY_CONTENT" \
