...
| コマンド | タイミング | JSONL type |
|---------|-----------|------------|
| `harnecess stop` | セッション終了時（自動） | `session.stop` |
| `harnecess snapshot` | 任意のタイミング（手動） | `session.snapshot` |

`snapshot` はセッションを停止せずにログを保存する。途中経過の記録や、問題発生時の状態保存に使う。
...
### session.stop / session.snapshot

セッションのメタデータ。`stop` は終了時、`snapshot` はオンデマンド。
...
```json
{
  ...
  "time": "2026-03-31T09:30:00Z",
  "session_id": "2026-03-31-e5f6g7h8",
  "agents": ["lead", "planner", "builder", "checker", "writer"]
}
```
